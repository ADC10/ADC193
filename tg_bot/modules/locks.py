    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("¡Mensajes {} bloqueados para todos los que no son administradores! ".format(args[0]))
                ...
                message.reply_text("¿Qué estás tratando de bloquear ...? Tratar /locktypes para la lista de bloqueables ")

    else:
        message.reply_text("No soy administrador o no tengo derechos de eliminación")
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("¿Qué intentas desbloquear ...? Pruebe /locktypes para la lista de bloqueables")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text("Veo un bot, y me han dicho que deje de unirse ..."
...
__help__ = """
 - /locktypes: una lista de posibles tipos de bloqueo

*Admin only:*
 - /lock <type>: bloquear elementos de cierto tipo (no disponible en privado)
 - /unlock <type>: desbloquear elementos de cierto tipo (no disponible en privado)
 - /locks: la lista actual de bloqueos en este chat.

Los bloqueos se pueden usar para restringir los usuarios de un grupo.
p.ej:
El bloqueo de URL eliminará automáticamente todos los mensajes con URL que no se hayan incluido en la lista blanca, las etiquetas de bloqueo eliminarán todo \
pegatinas, etc.
