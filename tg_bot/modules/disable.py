...
if is_module_loaded(FILENAME):
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text("Deshabilitó el uso de` {} `".format(disable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Ese comando no se puede deshabilitar")

        else:
            update.effective_message.reply_text("¿Qué debo desactivar?")
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text("Habilitado el uso de` {} `".format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("¿Eso está incluso deshabilitado?")

        else:
            update.effective_message.reply_text("¿Qué debo habilitar?")
    ...
    def list_cmds(bot: Bot, update: Update):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text("Los siguientes comandos son conmutables: \n {}".format(result),
                                                parse_mode=ParseMode.MARKDOWN)
        else:
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "No se pueden deshabilitar comandos".
    ...
    __help__ = """
- /cmds: verifica el estado actual de los comandos deshabilitados

* Solo administrador: *
  - /enable <cmd name>: habilita ese comando
  - /disable <cmd name>: deshabilita ese comando
  - /listcmds: enumera todos los comandos conmutables posibles
     "" "
