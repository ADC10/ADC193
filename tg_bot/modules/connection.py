    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if (var == "no"):
                ...
                update.effective_message.reply_text("Conexiones deshabilitadas a este chat para usuarios")
            elif(var == "yes"):
                sql.set_allow_connect_to_chat(chat.id, True)
                update.effective_message.reply_text("Conexiones habilitadas a este chat para usuarios")
            else:
                update.effective_message.reply_text("Por favor ingrese on/yes/off/ no en el grupo!")
        else:
            update.effective_message.reply_text("Por favor ingrese on/yes/off/ no en el grupo!")
    else:
        update.effective_message.reply_text("Por favor ingrese on/yes/off/ no en el grupo!")
    ...
    if update.effective_chat.type == 'private':
        if len(args) >= 1:
            try:
                ...
                update.effective_message.reply_text("¡Identificación de chat no válida proporcionada!")
            if (bot.get_chat_member(connect_chat, update.effective_message.from_user.id).status in ('administrator', 'creator') or 
                ...
                if connection_status:
                    ...
                    update.effective_message.reply_text("Conectado exitosamente a * {} *".format(chat_name), parse_mode=ParseMode.MARKDOWN)
                    ...
                    update.effective_message.reply_text("¡La conexión falló!")
            else:
                update.effective_message.reply_text("¡No se permiten conexiones a este chat!")
        else:
            update.effective_message.reply_text("¡Ingrese el ID de chat para conectarse!")
            history = sql.get_history(user.id)
            print(history.user_id, history.chat_id1, history.chat_id2, history.chat_id3, history.updated)

    else:
        update.effective_message.reply_text("¡Uso limitado solo a PM!")
...
def disconnect_chat(bot, update):
    if update.effective_chat.type == 'private':
        ...
        if disconnection_status:
            sql.disconnected_chat = update.effective_message.reply_text("¡Desconectado del chat!")
            #Rebuild user's keyboard
            keyboard(bot, update)
        else:
           update.effective_message.reply_text("¡Desconexión fallida!")
    else:
        update.effective_message.reply_text("Uso restringido solo a PMs")
        ...
        if (bot.get_chat_member(conn_id, user_id).status in ('administrator', 'creator') or 
            ...
            if need_admin == True:
                if bot.get_chat_member(conn_id, update.effective_message.from_user.id).status in ('administrator', 'creator') or user_id in SUDO_USERS:
                    ...
                    update.effective_message.reply_text("¡Necesitas ser administrador en un grupo conectado!")
                    exit(1)
            else:
                return conn_id
        else:
            update.effective_message.reply_text("El grupo cambió la conexión de derechos o ya no eres administrador. \n Te desconectaré".)
...
  - /connect <chatid>: conectarse al chat remoto
  - /disconnect: desconectarse del chat
  - /allowconnect on/yes/off/no: permite conectar usuarios al grupo
"" "
